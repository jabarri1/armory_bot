module ArmoryBot
  module Commands
    module Reaper
      ...
      command([:reaper, :Reaper, :REAPER], bucket: :overwatch, min_args: 4, rate_limit_message: 'All heroes share a rate limit. Wait %time% more seconds.') do |event, *account, region, platform, mode|
        ...
        data1 = HTTParty.get("https://api.lootbox.eu/#{platform}/#{region}/#{acc}/quick-play/hero/Reaper/", :verify => false ).parsed_response
        data2 = HTTParty.get("https://api.lootbox.eu/#{platform}/#{region}/#{acc}/competitive-play/hero/Reaper/", :verify => false ).parsed_response
        ...
        blossom = data["DeathBlossomKills"]
        blossom_most = data["DeathBlossomKills-MostinGame"]
        blossom_average = data["DeathBlossomKills-Average"]
        souls = data["SoulsConsumed"]
        souls_average = data["SoulsConsumed-Average"]
        souls_most = data["SoulsConsumed-MostinGame"]
        ...
        if data["statusCode"] == 500
          ...
          event.respond """#{event.user.mention} - #{name.capitalize} - Reaper - #{type}
```ruby
- Hero Specific -
Death Blossom Kills: #{blossom} | Most in Game: #{blossom_most} | Average: #{blossom_average}
Souls Consumed: #{souls} | Most in Game: #{souls_most} | Average: #{souls_average}
