module ArmoryBot
  module Commands
    module Dva
      ...
      command([:dva, :Dva, :DVa, :DVA], bucket: :overwatch, min_args: 4, rate_limit_message: 'All heroes share a rate limit. Wait %time% more seconds.') do |event, *account, region, platform, mode|
        ...
        data1 = HTTParty.get("https://api.lootbox.eu/#{platform}/#{region}/#{acc}/quick-play/hero/DVa/", :verify => false ).parsed_response
        data2 = HTTParty.get("https://api.lootbox.eu/#{platform}/#{region}/#{acc}/competitive-play/hero/DVa/", :verify => false ).parsed_response
        ...
        if data["statusCode"] == 500
          ...
          event.respond """#{event.user.mention} - #{name.capitalize} - D.Va - #{type}
```ruby
- Hero Specific -
